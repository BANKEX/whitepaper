...
\author{Yellow Paper, version 0.1.3 alpha}
...
\textbf{Value}. The smallest subdenomination of BKX is called Cent. All integer values are counted in Cent in the BankEx ecosystem. One BKX nomination is defined in Table~\ref{tab:tokens}.

\begin{table*}
    \caption{BKX Token Nomination}
...
\item[init]--- an unlimited syze byte array representing the EVM-code for account initialization procedure, same as in Ethereum, used when smart asset is created, $T_{init}$
\item[data]--- unlimited syze byte array representing input data of a message call transaction, similar to Ethereum, $T_{data}$
\end{description}
...
\textbf{Execution of smart-asset transaction}. Every operation involving smart assets will be using BankEx gas. We have developed system of fees, described in Table~\ref{tab:op-costs}.

\begin{table*}
    \caption{Operation costs}
    \label{tab:op-costs}\centering
    \begin{tabularx}{\textwidth}{|X|X|X|}
		...
		\hline
            \makecell[l]{Destruction of the Smart Asset} & \makecell{5 =<} & \makecell[l]{Amount depends on \\the Ethereum Network load} \\
		\hline
